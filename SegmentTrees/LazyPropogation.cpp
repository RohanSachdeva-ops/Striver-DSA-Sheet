...
class segTree{
  ...
  void updateInRange(int ind,int low,int high,int l,int r,int val){
    if(lazy[ind] != 0){
    ...
    int mid = low + (high-low)/2;
    updateInRange(2*ind+1,low,mid,l,r,val);
    updateInRange(2*ind+2,mid+1,high,l,r,val);
    seg[ind] = seg[2*ind+1]+seg[2*ind+2];
  }
...
int main() {
    ...
    while(q--){
      ...
      sg1.updateInRange(0,0,n-1,0,n-1,2);
      cout << (sg1.query(0,0,n-1,l,r)) << endl;
    }
}